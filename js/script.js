...
function abrirCarrito() {
  ...
  if (inputCupon) inputCupon.value = cuponAplicado;

  const mensaje = document.getElementById("cuponMensaje");
  if (mensaje) {
    mensaje.textContent = cuponAplicado ? `Cupón "${cuponAplicado}" aplicado.` : "";
...
function aplicarCupon() {
  ...
  if (codigo === "FELICES50") {
    ...
    mensaje.textContent = `Cupón "${codigo}" aplicado: 10% de descuento.`;
    mensaje.style.color = "#2e7d32";
  } else if (codigo === "") {
    cuponAplicado = "";
...
function renderizarCarrito() {
  ...
  const descuentoUsuario = usuarioActual ? (usuarioActual.porcentajeDescuento || 0) : 0;
  const descuentoCupon = cuponAplicado === "FELICES50" ? 0.10 : 0;
  const porcentajeDescuento = descuentoUsuario + descuentoCupon;
...
function comprarCarrito() {
  const mensaje = document.getElementById("compraMensaje");
  if (!mensaje) return;
